...
const SectionForm = forwardRef(({ data }, ref) => {
	const [menuData, setMenuData] = useState(datalists)
	...
	useEffect(() => {
		fetch('/sections')
			...
			.then(({ data }) => setMenuData(Object.keys(data).map((parentID) => data[parentID])))
	}, [])
	const close = (e) => {
		e.preventDefault()
		onClose()
				...
				<div id='section-switches'>
					<div className='form-switch form-check'>
						<input
							...
							ref={register}
						/>
						<label className='form-check-label' htmlFor='disable-section-checkbox'>
							Disable
						</label>
					</div>
					<div className='form-switch form-check'>
						<input
							...
							ref={register}
						/>
						<label className='form-check-label' htmlFor='section-visibility-checkbox'>
					...
					<div className='form-switch form-check'>
						<input
							...
							ref={register}
						/>
						<label className='form-check-label' htmlFor='time-restrict-section-checkbox'>

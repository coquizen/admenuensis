...
const SortableContainer = ({items, id, menuData }) => {
            ...
            <Container ref={setNodeRef}>
                <SortableContext items={itemIDs} strategy={verticalListSortingStrategy}>
                    {items?.map((item) => {
                        ...
                        if (item.subsections) return <SortableContainer key={item.id} id={item.id} items={item.subsections} menuData={menuData}/>
                        return (
                            <SortableItemWrapper dataID={item.id} key={item.id} id={item.id} menuData={menuData} formType={'item'}>{child}</SortableItemWrapper>
                    )
                    })}
                </SortableContext>
            </Container>
